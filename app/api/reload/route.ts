...
export async function POST(request: Request) {
  try {
    ...
    const espResponse = await fetch(ARDUINO_URL+"/refresh", {
      method: 'GET',
      
    });

    if (!espResponse.ok) {
      throw new Error('Failed to refresh data on ESP device');
    }

    const responseText = await espResponse.text();
    console.log('DATA refreshed:', responseText);

    return NextResponse.json(
      { success: true, message: "Refresh Succesfull"},
      { status: 200 }
    );
  } catch (error: unknown) {
